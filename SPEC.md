...
> [!WARNING]
> **ファイルベースルーティングの制約について**
> 開発者は設定ファイルやエントリーポイントを直接手書きする必要はありません。**`src/routes.nim` や `src/main.nim` を作成することは固く禁止されています。** これらのルーティングとサーバー起動ファイルは、Crown CLIによって裏側の `.crown/` ディレクトリに自動生成されます。開発者は `src/app/` 配下にページファイルを追加することだけに集中してください。
