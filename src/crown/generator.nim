...
proc generateRoutesCode*(appDir: string, isDev: bool = false): string =
  ...
  for e in entries:
    for methodTuple in e.methods:
      ...
      code &= &"        res = htmlResponse(crown.crownTiaraHtmlToString({e.importName}.{m}(req, {explicitArg})))\n"
      code &= &"      elif type({e.importName}.{m}(req, {explicitArg})) is Future[Html]:\n"
      code &= &"        res = htmlResponse(crown.crownTiaraHtmlToString(await {e.importName}.{m}(req, {explicitArg})))\n"
      ...
      code &= &"        res = htmlResponse(crown.crownTiaraHtmlToString({e.importName}.{m}(req)))\n"
      code &= &"      elif type({e.importName}.{m}(req)) is Future[Html]:\n"
      code &= &"        res = htmlResponse(crown.crownTiaraHtmlToString(await {e.importName}.{m}(req)))\n"
      ...
      code &= &"        res = htmlResponse(crown.crownTiaraHtmlToString({e.importName}.{m}(c, p)))\n"
      code &= &"      elif type({e.importName}.{m}(c, p)) is Future[Html]:\n"
      code &= &"        res = htmlResponse(crown.crownTiaraHtmlToString(await {e.importName}.{m}(c, p)))\n"
