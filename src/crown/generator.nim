...
proc generateCrownEnvPreserverModuleSource*(): string =
  ## Materialized as `.crown/crown_env_preserver.nim`. Basolato's env loader applies
  ## every `.env` key via ``putEnv``, which overwrites variables the Crown CLI set
  ## on the subprocess (notably incremental ``PORT``). Import this module strictly
  ## before ``import basolato`` so ``crownPortBeforeBasolatoEnv`` snapshots the CLI value.
